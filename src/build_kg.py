    ...
    for chunk in data:
        ...
        for ev in chunk.get("events", []):
            ...
            for e in ev.get("entities", []):
                ...
                # Type attribute — constrains virtual-entity binding at retrieval.
                attr = str(e.get("attribute", "") or "").strip().lower()
                if name:
...
def build_hypergraph(events: list[dict]) -> nx.MultiGraph:
    ...
    ent_attr_votes: dict[str, Counter] = defaultdict(Counter)  # canonical_lower → attribute vote counts
    ...
    for ev in events:
        for e in ev["entities"]:
            ...
            # Vote attributes across mentions; ignore "other"/empty.
            a = (e.get("attribute") or "").strip().lower()
            if a and a != "other":
                ent_attr_votes[key][a] += 1
    ...
    # Majority-vote attribute per entity; "other" when never typed.
    ent_attr: dict[str, str] = {
        k: (ent_attr_votes[k].most_common(1)[0][0] if ent_attr_votes[k] else "other")
        for k in ent_keys
    }
    ...
    for i, key in enumerate(ent_keys):
        G.add_node(
            ...
            attribute      = ent_attr[key],                           # type tag
